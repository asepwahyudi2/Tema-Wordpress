...
<article class="single-game single-software">
    <div class="single-game-header">
        <?php if (has_post_thumbnail()) { the_post_thumbnail('game-detail'); } else { echo '<div class="no-thumb single-no-thumb"><i class="fas fa-desktop"></i></div>'; } ?>
    ...
    <div class="single-game-body">
        ...
        if ($has_info) : ?>
        <div class="game-info-box software-info-box"><h3><i class="fas fa-circle-info"></i> Software Information</h3><div class="game-info-grid">
            <?php foreach ($fields as $key => $label) : $value = get_post_meta(get_the_ID(), $key, true); if ($value) : ?>
                <div class="game-info-item"><span class="label"><?php echo esc_html($label); ?></span><span class="value">
                    <?php if ($key === 'software_website') : ?><a href="<?php echo esc_url($value); ?>" target="_blank" rel="nofollow noopener">Official Site</a><?php else : echo esc_html($value); endif; ?>
                </span></div>
        ...
        <?php
        $requirements = array('soft_req_os'=>'OS','soft_req_cpu'=>'CPU','soft_req_ram'=>'RAM','soft_req_storage'=>'Storage');
        $has_req = false;
        foreach ($requirements as $key => $label) { if (get_post_meta(get_the_ID(), $key, true)) { $has_req = true; } }
        if ($has_req) : ?>
        <div class="system-req software-req"><h3><i class="fas fa-desktop"></i> Software Requirements</h3><div class="req-columns one-column"><div class="req-column"><h4>Required</h4><ul>
            <?php foreach ($requirements as $key => $label) : $value = get_post_meta(get_the_ID(), $key, true); if ($value) : ?>
        ...
        <?php
        $dl1 = get_post_meta(get_the_ID(), 'software_download_1', true);
        $dl2 = get_post_meta(get_the_ID(), 'software_download_2', true);
        $dl3 = get_post_meta(get_the_ID(), 'software_download_3', true);
        if ($dl1 || $dl2 || $dl3) : 
        ...
        ?>
        <div class="download-section software-download"><h3><i class="fas fa-download"></i> Download <?php the_title(); ?></h3><div class="download-buttons">
            <?php if ($dl1) : ?><a href="<?php echo esc_url($dl1); ?>" class="btn-download direct" target="_blank" rel="nofollow noopener"><i class="fas fa-download"></i> Official Download</a><?php endif; ?>
            ...
            <?php if ($dl3) : ?><a href="<?php echo esc_url($dl3); ?>" class="btn-download torrent" target="_blank" rel="nofollow noopener"><i class="fas fa-link"></i> Alternative</a><?php endif; ?>
        </div></div>
        <?php endif; ?>
...
if ($terms && !is_wp_error($terms)) :
    ...
    $related = new WP_Query(array(
        ...
        'posts_per_page' => 3,
        'orderby' => 'rand',
        'tax_query' => array(array('taxonomy'=>'software_category','field'=>'term_id','terms'=>$term_ids)),
    ));
    if ($related->have_posts()) : ?>
    <div class="related-games"><h3><i class="fas fa-desktop"></i> Related Software</h3><div class="related-grid">
